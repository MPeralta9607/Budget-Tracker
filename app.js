...
(function(){
  ...
  function renderGoal(){
    ...
    const goalName = data.goal ? (data.goal.name || '') : '';
    // Goal progress is driven by savings entries that match the goal name, plus the opening
    // amount the user said was already saved before using the app.
    const goalLinkedSaved = goalLinkedSavingsTotal();
    const opening = Math.max(0, data.openingSavings || 0);
    const withdrawn = Math.max(0, totalWithdrawnFromSavings());
    const goalSaved = Math.max(0, opening + goalLinkedSaved - withdrawn);
    const openingRemaining = Math.max(0, opening - withdrawn);
    const totalBalance = goalSaved;
    const contributedRemaining = Math.max(0, totalBalance - openingRemaining);
    const otherSavings = Math.max(0, currentSavingsBalance() - goalSaved);
    ...
    const balanceCaption = opening > 0
      ? `${fmt(openingRemaining)} already saved for ${goalName || 'this goal'} + ${fmt(contributedRemaining)} matched savings = ${fmt(totalBalance)} total.`
    ...
    const balanceBlock = `
      <div class="goal-head">
        <span class="goal-name">${escapeHtml(goalName || 'Total savings balance')}</span>
      ...
      <div class="goal-bar stacked-bar">
        ...
        ${contributedRemaining > 0 ? `<div class="goal-fill contributed-fill" style="width:${contributedPct}%"></div>` : ''}
      </div>
    ...
    const pct = Math.min(100, (totalBalance / data.goal.target) * 100);
    const reached = totalBalance >= data.goal.target;
    const remaining = Math.max(0, data.goal.target - totalBalance);
    ...
    goalEl.innerHTML = `
      ...
      <p class="goal-caption">${reached
        ? `You've hit your ${fmt(data.goal.target)} goal — ${fmt(totalBalance)} saved toward "${escapeHtml(data.goal.name || 'this goal')}". Consider setting a new one.`
        : `${fmt(totalBalance)} saved so far of ${fmt(data.goal.target)} — ${fmt(remaining)} to go.`}</p>
  ...
  function openGoalSheet(){
    ...
    $('openingSavingsNote').textContent = `Current saved amount: ${fmt(data.openingSavings || 0)}. Enter any extra money here and it will be added to that total.`;
  ...
  $('goalSaveBtn').addEventListener('click', async () => {
    // Opening savings is not a transaction — it never reduces this month's balance,
    // it's just what the person already had saved before using the app. It can be
    // saved on its own, independent of whether a goal target is also set.
