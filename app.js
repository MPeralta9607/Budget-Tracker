  ...
  function renderGoal(){
    ...
    goalEl.innerHTML = `
      ...
      <p class="goal-caption">${reached
        ? `You've hit your ${fmt(data.goal.target)} goal — ${fmt(goalSaved)} saved toward "${escapeHtml(data.goal.name || 'this goal')}". Consider setting a new one.`
        : `${fmt(goalSaved)} saved so far of ${fmt(data.goal.target)} — ${fmt(remaining)} to go.`}</p>
