...
(function(){
  ...
  const UI_STATE_KEY = 'finance-tracker-ui-state';
  let uiState = { expensesCollapsed:false, recurringCollapsed:false, collapsedSections:{} };

  function loadUiState(){
    try{
      const saved = JSON.parse(localStorage.getItem(UI_STATE_KEY) || '{}');
      uiState = {
        expensesCollapsed: saved.expensesCollapsed === true,
        recurringCollapsed: saved.recurringCollapsed === true,
        collapsedSections: saved.collapsedSections && typeof saved.collapsedSections === 'object' ? saved.collapsedSections : {}
      };
    }catch(e){
      uiState = { expensesCollapsed:false, recurringCollapsed:false, collapsedSections:{} };
    }
  }
  ...
  function setSectionCollapsed(section, collapsed){
    if(section === 'recurring') uiState.recurringCollapsed = collapsed;
    else uiState.collapsedSections[section] = collapsed;
    saveUiState();
    const button = document.querySelector(`[data-collapse-section="${section}"]`);
    const card = button ? button.closest('.collapsible-section') : null;
    if(card) card.classList.toggle('is-collapsed', collapsed);
    if(button) button.textContent = collapsed ? 'Expand' : 'Collapse';
  }

  function initSectionCollapse(){
    document.querySelectorAll('[data-collapse-section]').forEach(button => {
      const section = button.getAttribute('data-collapse-section');
      const collapsed = section === 'recurring' ? uiState.recurringCollapsed : uiState.collapsedSections[section] === true;
      setSectionCollapsed(section, collapsed);
      button.addEventListener('click', () => {
        const current = section === 'recurring' ? uiState.recurringCollapsed : uiState.collapsedSections[section] === true;
  ...
  function sanitizeMoneyEntry(raw){
    const text = String(raw || '').replace(/[^\d.]/g, '');
    const dotIndex = text.indexOf('.');
    if(dotIndex === -1) return text;
    const whole = text.slice(0, dotIndex);
    const cents = text.slice(dotIndex + 1).replace(/\./g, '');
    return `${whole}.${cents}`;
  }
  function formatMoneyEntry(raw){
    const text = sanitizeMoneyEntry(raw).trim();
    if(!text) return '';
    if(text === '.') return '0.00';
  ...
  function parseMoneyEntry(raw){
    const text = sanitizeMoneyEntry(raw).trim().replace(/[$, ]/g, '');
    if(!text) return NaN;
    const value = Number(text);
            ...
            if(type === 'income'){
              ...
              if(!skipped.has(key) && !existingExpense.has(key)){
                data.expenses.push({
                  ...
                  paymentMethod: (type === 'expense' || type === 'savings') ? (rule.paymentMethod || 'other') : undefined,
                  date: iso,
                  recurringId: rule.id,
                  recurringDate: iso,
                  recurringLabel: label
  ...
  function renderHeader(){
    ...
    if(budget === 0){
      ...
      $('tapeCaption').textContent = isCurrent
        ? `Day ${dayOfMonth} of ${daysInMonth} - ${fmt(spent)} spent of ${fmt(budget)} income budget (${usedPct}%)`
        : `${daysInMonth} days - ${fmt(spent)} spent of ${fmt(budget)} income budget (${usedPct}%)`;
    }
  }
  ...
  function renderRule(){
    ...
    const leftover = income - (totals.needs + totals.wants);
    const impliedSavings = Math.max(0, leftover);
  ...
  function renderCategoryBudgets(){
    ...
    listEl.innerHTML = rows.map(r => {
      ...
      return `
        <div class="cat-budget-row" role="button" tabindex="0" data-category-expenses="${escapeHtml(r.c.id)}" aria-label="View ${escapeHtml(r.c.label)} expenses">
          <div class="cbr-head">
    ...
    listEl.querySelectorAll('[data-category-expenses]').forEach(row => {
      const open = () => openCategoryExpenses(row.getAttribute('data-category-expenses'));
  ...
  function openCategoryExpenses(categoryId){
    const category = catInfo(categoryId);
    const expenses = budgetableExpenses(expensesFor(viewYear, viewMonth)).filter(e => e.category === categoryId);
    const total = expenses.reduce((sum, entry) => sum + entry.amount, 0);
    $('categoryExpenseTitle').textContent = `${category.label} expenses`;
    $('categoryExpenseSub').textContent = `${MONTH_NAMES[viewMonth]} ${viewYear}`;
    $('categoryExpenseList').innerHTML = expenses.length
      ? `<div class="category-expense-total"><span>Total spent</span><strong>${fmt(total)}</strong></div>${expenses.map(e => {
          const date = new Date(e.date + 'T00:00:00').toLocaleDateString('en-US', {month:'short', day:'numeric'});
          return `<div class="expense-row"><span class="cat-dot" style="background:${category.color}"></span><div class="exp-main"><div class="exp-note">${e.note ? escapeHtml(e.note) : category.label}</div><div class="exp-cat">${date}${e.paymentMethod ? ` · ${escapeHtml(paymentMethodLabel(e.paymentMethod))}` : ''}</div></div><span class="exp-amt">${fmt(e.amount)}</span></div>`;
        }).join('')}`
      : '<p class="cat-budget-empty">No expenses in this category for this month.</p>';
    $('categoryExpenseSheet').classList.add('open');
  }

  function closeCategoryExpenses(){
    const sheet = $('categoryExpenseSheet').querySelector('.sheet');
    sheet.style.transform = '';
    sheet.style.transition = '';
    $('categoryExpenseSheet').classList.remove('open');
  }

  (function initSheetSwipe(){
    const closeHandlers = {
      sheet: closeSheet,
      budgetSheet: closeBudgetSheet,
      catBudgetSheet: closeCatBudgetSheet,
      goalSheet: closeGoalSheet,
      categoryExpenseSheet: closeCategoryExpenses
    };
    document.querySelectorAll('.sheet-overlay').forEach(overlay => {
      const sheet = overlay.querySelector('.sheet');
      const handle = overlay.querySelector('.sheet-handle');
      const close = closeHandlers[overlay.id];
      if(!sheet || !handle || !close) return;
      let startY = 0;
      let currentY = 0;
      let tracking = false;
      ...
      const beginSwipe = (clientY, pointerId = null) => {
      ...
      const updateSwipe = (clientY, event) => {
        if(!tracking) return;
        currentY = clientY;
        const distance = Math.max(0, currentY - startY);
        if(distance > 0){
      ...
      handle.addEventListener('pointerdown', event => {
        beginSwipe(event.clientY, event.pointerId);
        handle.setPointerCapture(event.pointerId);
      });
      handle.addEventListener('touchstart', event => {
        beginSwipe(event.touches[0].clientY);
      }, { passive:true });
      window.addEventListener('pointermove', event => {
        if(!tracking || (activePointerId !== null && event.pointerId !== activePointerId)) return;
        updateSwipe(event.clientY, event);
      }, { passive:false });
      window.addEventListener('touchmove', event => {
        if(!tracking) return;
        updateSwipe(event.touches[0].clientY, event);
      }, { passive:false });
      window.addEventListener('pointerup', event => {
        if(tracking && (activePointerId === null || event.pointerId === activePointerId)) endSwipe();
      });
      ...
      handle.addEventListener('pointerup', event => {
        if(event.pointerId === activePointerId) endSwipe();
      });
  ...
  function renderRecurringList(){
    const recurringCard = $('toggleRecurring').closest('.recurring-card');
    recurringCard.classList.toggle('collapsed', uiState.recurringCollapsed);
    $('toggleRecurring').textContent = uiState.recurringCollapsed ? 'Expand' : 'Collapse';
    ...
    listEl.innerHTML = active
      ...
      .map(rule => {
        ...
        if((rule.type === 'expense' || rule.type === 'savings') && rule.paymentMethod){
          metaParts.push(paymentMethodLabel(rule.paymentMethod));
        }
  ...
  function renderExpenseList(){
    ...
    listEl.classList.toggle('all-collapsed', uiState.expensesCollapsed);
    $('toggleExpenseGroups').textContent = uiState.expensesCollapsed ? 'Expand all' : 'Collapse all';
    const allExps = expensesFor(viewYear, viewMonth);
    $('expenseMonthTotal').textContent = fmt(allExps.reduce((sum, entry) => sum + entry.amount, 0));
    renderPaymentSummary(allExps);
    ...
    listEl.innerHTML = dateKeys.map(dateKey => {
      ...
      const rows = groups[dateKey].map(e => {
        ...
        const paymentMethod = paymentMethodLabel(e.paymentMethod);
        return `
          <div class="expense-row" data-id="${e.id}">
            <span class="cat-dot" style="background:${c.color}"></span>
            <div class="exp-main">
              <div class="exp-note">${e.note ? escapeHtml(e.note) : c.label}</div>
              <div class="exp-cat">${c.label}${paymentMethod ? `<span class="exp-badge">${escapeHtml(paymentMethod)}</span>` : ''}${e.recurringId ? '<span class="exp-badge">Recurring</span>' : ''}${isWithdrawal(e) ? '<span class="exp-badge savings-badge">From savings</span>' : ''}</div>
            </div>
      ...
      return `<div class="day-group"><div class="day-label">${label}<button type="button" class="expense-group-toggle" aria-expanded="true">Collapse</button></div><div class="day-group-rows">${rows}</div></div>`;
    }).join('');

    listEl.querySelectorAll('.expense-group-toggle').forEach(button => {
      button.addEventListener('click', () => {
        const group = button.closest('.day-group');
  ...
  function renderPaymentSummary(expenses){
    const totals = { credit_card:0, yappy:0, other:0 };
    (expenses || []).forEach(entry => { totals[entry.paymentMethod || 'other'] = (totals[entry.paymentMethod || 'other'] || 0) + entry.amount; });
    $('paymentSummary').innerHTML = Object.entries(PAYMENT_METHOD_LABELS).map(([method, label]) => `<div class="payment-total"><span class="payment-label">${label}</span><span class="payment-amount">${fmt(totals[method] || 0)}</span></div>`).join('');
  }
  ...
  function renderAdvice(chartInfo){
    ...
    if(income > 0){
      ...
      if(surplus > 0 && surplusPct >= 25){
        ...
        tips.push({ type:'good', text:`You've spent ${fmt(spent)} of ${fmt(income)} income so far, leaving ${fmt(surplus)} unallocated. Consider moving some of it into savings${goalNote} before it gets absorbed into next month's spending.` });
      }
  ...
  $('categoryExpenseCloseBtn').addEventListener('click', closeCategoryExpenses);
  $('categoryExpenseSheet').addEventListener('click', e => { if(e.target.id === 'categoryExpenseSheet') closeCategoryExpenses(); });
  $('toggleExpenseGroups').addEventListener('click', () => {
    const groups = document.querySelectorAll('#expenseList .day-group');
    const shouldCollapse = Array.from(groups).some(group => !group.classList.contains('collapsed'));
    $('expenseList').classList.toggle('all-collapsed', shouldCollapse);
    uiState.expensesCollapsed = shouldCollapse;
    saveUiState();
    groups.forEach(group => {
      group.classList.toggle('collapsed', shouldCollapse);
      const button = group.querySelector('.expense-group-toggle');
      if(button){
        button.textContent = shouldCollapse ? 'Expand' : 'Collapse';
        button.setAttribute('aria-expanded', String(!shouldCollapse));
      }
    });
    $('toggleExpenseGroups').textContent = shouldCollapse ? 'Expand all' : 'Collapse all';
  });
  ...
  $('inpAmount').addEventListener('input', () => {
    const input = $('inpAmount');
    const sanitized = sanitizeMoneyEntry(input.value);
    if(sanitized !== input.value) input.value = sanitized;
  });
  $('inpAmount').addEventListener('blur', () => {
    const input = $('inpAmount');
    const formatted = formatMoneyEntry(input.value);
    input.value = formatted || '0.00';
  });
  $('inpAmount').addEventListener('focus', () => {
    if($('inpAmount').value === '0.00'){
      $('inpAmount').setSelectionRange(0, $('inpAmount').value.length);
    }
  });
  ...
  function setEntryMode(mode){
    ...
    $('paymentMethodField').style.display = (mode==='expense' || mode==='savings') ? 'block' : 'none';
  ...
  function openSheet(mode, entry=null, opts={}){
    ...
    setSheetTitle();
    $('inpAmount').value = entry ? formatMoneyEntry(String(entry.amount || 0)) : '0.00';
    ...
    $('inpPaymentMethod').value = entry && entry.paymentMethod ? entry.paymentMethod : 'credit_card';
  ...
  $('saveBtn').addEventListener('click', async () => {
    const amount = parseMoneyEntry($('inpAmount').value);
    ...
    function resolvePaymentMethod(){
      return $('inpPaymentMethod').value || 'other';
    }
    ...
    if(editingEntry){
      if(editingEntry.type === 'expense'){
        ...
        if(item){
          ...
          item.paymentMethod = resolvePaymentMethod();
        }
      ...
      if(updatingRecurringRule){
        ...
        if(rule){
          ...
          if(editingEntry.type === 'expense' || editingEntry.type === 'savings') rule.paymentMethod = resolvePaymentMethod();
      ...
      if(creatingRecurring){
        ...
        const rule = { id: ruleId, type: 'expense', amount, category, note, fundedBy, paymentMethod, startDate: date, repeatInterval, repeatUnit, untilDate, active: true };
        if(repeatUnit === 'weeks') rule.weekDays = selectedWeekDays;
        data.recurring.push(rule);
        data.expenses.push({ id: 'e'+Date.now()+Math.random().toString(36).slice(2,7), amount, category, note, fundedBy, paymentMethod, date, recurringId: ruleId, recurringDate: date, recurringLabel: recurringFrequencyLabel(rule) });
      } else {
        data.expenses.push({ id: 'e'+Date.now()+Math.random().toString(36).slice(2,7), amount, category, note, fundedBy, paymentMethod, date });
      }
      ...
      if(creatingRecurring){
        const ruleId = 'r' + Date.now() + Math.random().toString(36).slice(2,7);
        const rule = { id: ruleId, type: 'savings', amount, note, paymentMethod, startDate: date, repeatInterval, repeatUnit, untilDate, active: true };
        if(repeatUnit === 'weeks') rule.weekDays = selectedWeekDays;
        data.recurring.push(rule);
        data.expenses.push({ id: 'e'+Date.now()+Math.random().toString(36).slice(2,7), amount, category:'savings', note, paymentMethod, date, recurringId: ruleId, recurringDate: date, recurringLabel: recurringFrequencyLabel(rule) });
      } else {
        data.expenses.push({ id: 'e'+Date.now()+Math.random().toString(36).slice(2,7), amount, category:'savings', note, paymentMethod, date });
      }
    }
  ...
  function openGoalSheet(){
    ...
    $('openingSavingsClearBtn').disabled = !(data.openingSavings > 0);
  ...
  $('openingSavingsClearBtn').addEventListener('click', async () => {
    if(!(data.openingSavings > 0)) return;
    const ok = confirm(`Remove ${fmt(data.openingSavings)} of already saved money?`);
    if(!ok) return;
    data.openingSavings = 0;
    await saveData();
    closeGoalSheet();
    renderAll();
  });
