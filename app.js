...
(function(){
  ...
  function renderGoal(){
    ...
    // Goal progress is driven by actual savings on hand after any emergency withdrawals.
    const contributed = Math.max(0, totalSaved());
    const opening = Math.max(0, data.openingSavings || 0);
    const withdrawn = Math.max(0, totalWithdrawnFromSavings());
    const openingRemaining = Math.max(0, opening - withdrawn);
    const withdrawnFromContributed = Math.max(0, withdrawn - opening);
    const contributedRemaining = Math.max(0, contributed - withdrawnFromContributed);
    const totalBalance = openingRemaining + contributedRemaining;
    ...
    const balanceCaption = opening > 0
      ? `${fmt(openingRemaining)} left from your original savings + ${fmt(contributedRemaining)} saved since = ${fmt(totalBalance)} total.`
      : `${fmt(totalBalance)} saved so far.`;
    const withdrawnNote = withdrawn > 0 ? ` (${fmt(withdrawn)} withdrawn for expenses is already subtracted.)` : '';

    const balanceBlock = `
      <div class="goal-head">
        <span class="goal-name">Total savings balance</span>
    ...
    goalEl.innerHTML = `
      ...
      <p class="goal-caption">${reached
        ? `You've hit your ${fmt(data.goal.target)} goal — ${fmt(totalBalance)} total savings toward "${escapeHtml(data.goal.name || 'this goal')}". Consider setting a new one.`
